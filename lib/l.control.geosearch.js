...
L.Control.GeoSearch = L.Control.extend({
    ...
    setConfig: function (options) {
        this._config = {
            ...
            'searchLabel': options.searchLabel || 'búsqueda por dirección...',
            'notFoundMessage' : options.notFoundMessage || 'La dirección especificada no se ha podido encontrar.',
        ...
        try {
            ...
            if(typeof provider.GetLocations == 'function') {
                var results = provider.GetLocations(qry, function(results) {
                    this._processResults(qry, results);
                }.bind(this));
            }
            else {
    ...
    _processResults: function(qry, results) {
        if (results.length == 0)
            throw this._config.notFoundMessage;

        this._map.fireEvent('geosearch_foundlocations', {Locations: results});
        this._showLocation(qry, results[0]);
    },

    _showLocation: function (qry, location) {
        if (typeof this._positionMarker === 'undefined')
            this._positionMarker = L.marker([location.Y, location.X], { bounceOnAdd: true }).addTo(this._map);
        else
