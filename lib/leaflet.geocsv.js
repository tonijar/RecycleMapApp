...
L.GeoCSV = L.GeoJSON.extend({
	...
	addData : function(data) {
		if ( typeof data === "string") {
			...
			for (var i = 0; i < titulos.length; i++) {
				...
				if (prop == "" || prop == "_" || this._propertiesNames.indexOf(prop) >= 0)
					prop = "prop-" + i;
	...
	_deleteDobleQuotes : function(cadena) {
		if (this.options.deleteDobleQuotes)
	...
	_csv2json : function(csv) {
		...
		for (var num_linea = 0; num_linea < csv.length; num_linea++) {
			var campos = csv[num_linea].trim().split(this.options.fieldSeparator), lng = parseFloat(campos[titulos.indexOf("lng")]), lat = parseFloat(campos[titulos.indexOf("lat")]);
